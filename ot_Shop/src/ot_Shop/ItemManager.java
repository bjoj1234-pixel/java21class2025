...
public class ItemManager {
	...
	public void addItem(String name, int price, String category) {
		itemList.add(new Item(name,price,category));//메소드화 시켜서 아래 new Item()을 안써도 되게 간추림.
	}
	...
	// ------------- 장바구니 ----------------
	// usid => id, caId => 카테고리 index 번호, itemID => item index번호
	public void addCart(String usid, int caId, int itemID) {
		...
		int log = user.logindex; //UserMenu 클래스의 로그인된 log값을 static로 공유함

		//개발에서 temp의 의미는 임시 기억장소를 뜻한다.
		Cart temp = new Cart();
		temp.userId = usid;
		...
		for(int i=0; i<itemList.size(); i++) {
			...
			if(itemList.get(itemID).name.equals(itemList.get(i).name)){
				...
				for(int j=0; j<jangList.size(); j++) {
					//비교 => 현재 장바구니에 담긴 아이템 이름과 아이템리스트 이름이 같은지
	...
	public void addItem02(String addname, int addprice, int addcategory) {
		// Item 클래스 추가 => add
		Item temp = new Item(addname,addprice,category.get(0));
		itemList.add(temp);	
	}	
