		...
		if(sel==1) {
			...
			if(inwon >=1 && inwon <= 8) {
				...
				if(inwon ==1) {				
					...
					int idx = scan.nextInt()-1; // 인덱스 번호라 미리 1을 빼주고 시작
			...
			// --- [예매 인원이 2명 이상일 경우] ----
				...
				// selectCnt가 최초에 선택한 inwon보다 작으면 아직 좌석선택이 다 끝나지 않았음을 조건으로 표시
				...
				while(selectCnt < inwon) {
					...
					if(inwon - selectCnt == 1) {
						...
						//셋팅 부분
						//현재 선택한 좌석과 바로 옆 좌석을 '선택중[0]' 상태로 변경
						seat[idx1]=1;
						seat[idx1+1]=1;
